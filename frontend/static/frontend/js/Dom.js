...
(function () {
    ...
    Bee.Dom = {
        ...
        getElementsByAttribute: function (attribute, value, single, ignoreList) {
            ...
            for (; i < n; i++) {
                if (Bu.defined(allElements[i].getAttribute(attribute))) {  // Element exists with attribute. Add to array.
                    if (value) {
        ...
        getElementsByName: function (name) {
            ...
            for (; i < n; i++) {
                if (allElements[i].tagName.toLowerCase() === name) {  // Element exists with attribute. Add to array.
                    matchingElements.push(allElements[i]);
                }
            }
            return matchingElements;
        ...
        closeWin: function (windowEl, remove, fade, delay) {
            var self = this;
            if (windowEl && this.getDisplayState(windowEl) === 1) {//console.log(fade);
        ...
        setBackGroundPosition: function (el, posX = "", posY = "") {
            if (el.style.backgroundPositionX) {//other browsers apart from FFx
        ...
        createItems: function (items, isSub) {
            ...
            Bo.forEach(items, function (item) {
                html += '<li>';
                if (typeof(item.sub) === 'object') { // An array will return 'object'
                    if (isSub) {
